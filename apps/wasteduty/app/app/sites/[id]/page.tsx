...
export default async function SiteDetailPage({
  ...
  const found = await withOrg(db, ctx.orgId, async (tx) => {
    const record = await getRecord(tx, "site_record", id);
  ...
  const d = record.data as SiteRecord;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <section className="grid gap-2 rounded border border-border p-4 text-sm sm:grid-cols-2">
        <p>
          <span className="text-muted-foreground">Permit:</span> {d.permitRef}
        </p>
        <p>
          <span className="text-muted-foreground">Tonnage limit:</span>{" "}
          {d.tonnageLimit} t
        </p>
        <p>
        ...
        <p>
          <span className="text-muted-foreground">EWC codes:</span>{" "}
          {d.permittedEwcCodes.join(", ")}
        </p>
      </section>
      ...
      <section>
        ...
        {evidence.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No evidence attached — e.g. the permit document.
          </p>
        ) : (
