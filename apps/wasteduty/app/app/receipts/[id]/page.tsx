...
export default async function ReceiptDetailPage({
  ...
  const found = await withOrg(db, ctx.orgId, async (tx) => {
    const record = await getRecord(tx, "waste_receipt", id);
  ...
  const d = record.data as WasteReceipt;
    ...
    <div className="flex flex-col gap-8">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-semibold">
          Movement {d.transferDate} — {d.ewcCode}
        </h1>
      ...
      <section className="grid gap-2 rounded border border-border p-4 text-sm sm:grid-cols-2">
        ...
        <p>
          <span className="text-muted-foreground">Carrier:</span> {d.carrierRef}
        </p>
        <p>
          <span className="text-muted-foreground">Origin:</span> {d.origin}
        </p>
        <p>
        ...
        <p>
          <span className="text-muted-foreground">DWT ref:</span>{" "}
          {d.dwtSubmissionRef ?? "not yet recorded"}
        </p>
      </section>
      ...
      <section>
        ...
        {evidence.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No evidence attached. Files are hashed (SHA-256) and immutable once
            attached.
          </p>
        ) : (
            ...
            {evidence.map((e) => (
              <li key={e.id} className="flex items-center gap-4 p-3 text-sm">
                ...
                <span className="text-xs text-muted-foreground">
                  {(e.sizeBytes / 1024).toFixed(1)} KB · sha256{" "}
                  {e.sha256.slice(0, 12)}…
                </span>
              </li>
            ))}
